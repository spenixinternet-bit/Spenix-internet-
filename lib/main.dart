          ...
          : ListView.builder(
              ...
              itemBuilder: (context, i) {
                ...
                return Container(
                  ...
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: const Color(0xFF1A1F3A), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.cyan.withOpacity(0.2))),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(children: [Expanded(child: Text(p['name'], style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold))), Text('UGX ${p['price']}', style: const TextStyle(color: Colors.cyan, fontWeight: FontWeight.bold))]),
                      const SizedBox(height: 8),
                      Text('${p['durationDays']} days', style: TextStyle(color: Colors.grey[400])),
                      const SizedBox(height: 12),
                      ElevatedButton(onPressed: () => Get.to(() => PaymentScreen(package: p)), style: ElevatedButton.styleFrom(backgroundColor: Colors.cyan, minimumSize: const Size(double.infinity, 44)), child: const Text('Subscribe')),
                    ],
                  ),
                );
              },
...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  Future<void> _loadPhone() async {
    ...
    if (user != null) phone.text = user['phone'] ?? '';
  }
...
class _VoucherScreenState extends State<VoucherScreen> {
  ...
  Future<void> apply() async {
    ...
    await db.addSubscription(sub);
    await db.addPayment({'id': const Uuid().v4(), 'userId': user['id'], 'subscriptionId': sub['id'], 'amount': 0, 'method': 'voucher', 'status': 'success', 'voucherCode': c, 'createdAt': DateTime.now().toIso8601String()});
    setState(() => loading = false);
